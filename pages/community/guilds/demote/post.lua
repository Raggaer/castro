...
function post()
    ...
    if tonumber(character.level) ~= 2 then
        http:redirect("/")
        return
    end

    local nextRank = db:singleQuery("SELECT id FROM guild_ranks WHERE level = 1 AND guild_id = ?", guild.id)
    ...
    session:setFlash("success", "Member demoted")
