function get()
    ...
    if data.list ~= nil then
        for i, forum in pairs(data.list) do
            ...
            data.list[i].last = db:singleQuery("SELECT a.title as title, b.name as name, a.created_at as created FROM castro_forum_post a, players b WHERE a.author = b.id AND a.category_id = ?", forum.id)
